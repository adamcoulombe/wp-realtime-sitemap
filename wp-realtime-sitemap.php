...
Description: Hacked Version of Daniel Tweedy's Realtime sitemap plugin. Adds a sitemap to your Wordpress blog that is always up-to-date. Add `[wp-realtime-sitemap]` to any page or post and the site map will be added there. Use Settings->WP Realtime Sitemap to set options.
Version: 1.5.4
			...
			if (preg_match('|wp-realtime-sitemap|', $code)) {
				...
				if (($all_options['show_pages'] != 'No' && $all_options['show_pages'] != 'Off') && ($show == 'pages' || $show == 'all')) {
					$pages_header = (empty($pages_header)) ? __('Pages', 'wp-realtime-sitemap') : $pages_header;
						...
						$pages = '<div id="wp-realtime-sitemap-pages">';
						
					if($pages_header != 'none'){
						$pages .= '<h3>' . $pages_header . '</h3>';
					}
					...
					if ($all_options['post_show_categories'] != 'No') {
						...
						if (count($thecategories) > 0) {
							foreach($thecategories as $category) {
								...
								$posts .= '<li><a href="' . get_category_link($category->term_id) . '" >' . $category->name . '</a><ul>';
							...
							$posts_beginning = '<div id="wp-realtime-sitemap-posts">';
							if($posts_header != 'none'){
								$posts_beginning .= '<h3>' . $posts_header . '</h3>';
							}
							
							$posts = $posts_beginning . '<ul>' . $posts . '</ul></div>';
				...
				if (($all_options['show_archives'] != 'No' && $all_options['show_archives'] != 'Off') && ($show == 'archives' || $show == 'all')) {
					...
					$archives = '<div id="wp-realtime-sitemap-archives">';
					if($archives_header!='none'){
					$archives .= '<h3>' . $archives_header . '</h3>';
					}
				...
				if (($all_options['show_categories'] != 'No' && $all_options['show_categories'] != 'Off') && ($show == 'categories' || $show == 'all')) {
					...
					$categories = '<div id="wp-realtime-sitemap-categories">';
					if($categories_header!='none'){
						$categories .= '<h3>' . $categories_header . '</h3>';
					}
				...
				if (($all_options['show_tags'] != 'No' && $all_options['show_tags'] != 'Off') && ($show == 'tags' || $show == 'all')) {
					...
					$tags = '<div id="wp-realtime-sitemap-tags">';
					if($tags_header!='none'){
					$tags .= '<h3>' . $tags_header . '</h3>';
					}
					// Tag Cloud: Yes/No?
